...
export const messageLogs = pgTable(
  'message_logs',
  {
    ...
    scheduledSendTime: timestamp('scheduled_send_time', {
      withTimezone: true,
      mode: 'date',
    }).notNull(),
    ...
    // Status tracking
    status: varchar('status', { length: 20 }).notNull().default(MessageStatus.SCHEDULED),
  ...
  (table) => ({
    ...
    userIdIdx: index('idx_message_logs_user_id').on(table.userId),

    // Index: status (for filtering by status)
    statusIdx: index('idx_message_logs_status').on(table.status),

    // Index: scheduled_send_time (for time-based queries)
    scheduledTimeIdx: index('idx_message_logs_scheduled_time').on(table.scheduledSendTime),
    ...
    schedulerIdx: index('idx_message_logs_scheduler').on(
      table.messageType,
      table.status,
      table.scheduledSendTime
    ),
    ...
    recoveryIdx: index('idx_message_logs_recovery')
      ...
      .where(sql`${table.status} IN ('SCHEDULED', 'RETRYING', 'FAILED')`),

    // Unique constraint: idempotency_key
    // Database-level enforcement to prevent duplicate messages
    idempotencyUniqueIdx: unique('idx_message_logs_idempotency').on(table.idempotencyKey),
  })
);
